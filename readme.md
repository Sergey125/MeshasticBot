...
Умный и невероятно отказоустойчивый шлюз между радиосетью Meshtastic (на базе Heltec/ESP32) и Telegram. Бот работает в связке с n8n для выполнения сложных логических задач (ИИ, метео) и управляется прямо из Telegram с помощью удобных интерактивных кнопок.

## 🌟 Главные фичи
- **Интерактивное меню (Inline Buttons):** Бот сам сканирует доступные каналы рации и предлагает их в виде кнопок для отправки. Никаких команд вида `/c 1`!
- **Умная память (Smart DM):** Бот запоминает 5 последних человек, писавших в радиоэфир, и выводит кнопки для быстрой отправки им личных сообщений.
- **Интеграция с n8n:** Поддержка команд `@ии` (запрос к нейросетям) и `метео` (запрос погоды по GPS координатам отправителя).
- **Пинг-Понг:** Встроенный калькулятор дистанции по команде `пинг` (бот отвечает только в личку или доп. каналы, чтобы не спамить в LongFast).
- **Бронебойная защита от сбоев (Auto-Recovery):**
  - **Network Watchdog:** Пингует рацию каждые 3 секунды.
  - **Keep-Alive:** Контроль зависших сокетов на уровне ядра Linux.
  - **Thread Pulse:** Мониторинг внутренних процессов библиотеки Meshtastic.
- **Ежедневные отчеты:** Каждое утро в 07:00 бот присылает в ТГ краткую сводку об аптайме и количестве предотвращенных сбоев за сутки.
- **Системный статус:** Команда `/test` для ручной проверки связи с рацией.

## ⚙️ Установка и запуск
1. Клонируйте репозиторий.
2. Установите зависимости: `pip install requests telebot meshtastic pypubsub`
3. Впишите свои данные (IP рации, Telegram Token, Admin ID) в верхнюю часть файла `bridge_bot.py`.
4. Настройте `meshtastic-bot.service` и скопируйте его в `/etc/systemd/system/`.
5. Выполните: `sudo systemctl enable meshtastic-bot && sudo systemctl start meshtastic-bot`.
